...
public class MdcContextLifterConfiguration {
	...
	public static class MdcContextLifter<T> implements CoreSubscriber<T> {
		...
		/**
		 * Extension function for the Reactor [Context]. Copies the current context to the
		 * MDC, if context is empty clears the MDC. State of the MDC after calling this
		 * method should be same as Reactor [Context] state. One thread-local access only.
		 */
